...
const IntoScreen = () => {
  ...
  const handleAppleSignIn = async () => {
    ...
    setLoadingProvider(null);
    if (error) {
      setErrorModal({ visible: true, title: "Apple Sign-In Failed", message: error });
      return;
    }
    if (user) {
      router.replace(user.displayName ? "/(tabs)" : "/(auth)/user_name_screen");
  ...
  const content = (
    <View style={isWeb ? webStyles.content : styles.content}>
      ...
      <View style={isWeb ? webStyles.buttons : styles.buttons}>
        <AuthBtn title="Continue with email address" onPress={() => router.push("/(auth)/login_screen")} disabled={isLoading} />
        <AuthBtn title="Continue with Google" onPress={handleGoogleSignIn} imgSource={require("@/assets/images/google-icon.png")} disabled={isLoading} loading={loadingProvider === "google"} />
        {(Platform.OS === "ios" || Platform.OS === "web") && (
          <AuthBtn title="Continue with Apple" onPress={handleAppleSignIn} imgSource={require("@/assets/images/apple.png")} disabled={isLoading} loading={loadingProvider === "apple"} />
        )}
      </View>

      <View style={styles.bottomRow}>
        <Text style={{ fontWeight: "400", fontSize: 16 }}>Don't have an account?</Text>
        <TouchableOpacity onPress={() => router.push("/(auth)/signup_screen")}>
          <Text style={{ fontWeight: "700", fontSize: 16, color: "#007AFF" }}>Sign Up</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  content: { flex: 1, padding: 16, alignItems: "center", justifyContent: "center", gap: 12 },
  ...
  loadingBox: { backgroundColor: "#fff", borderRadius: 16, paddingVertical: 30, paddingHorizontal: 40, alignItems: "center", gap: 12 },
